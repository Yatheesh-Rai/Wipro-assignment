...
def _get_Sonar_UTcoverage(job):
    ...
    try:
        ...
        logger.error("%s doesnt' have pom file" % job)
        return None
...
def save_csv(reports):
    ...
    csv_writer.writerow(["Job Name", "CheckStyle", "PMD", "Findbugs",
                         "last snapshot build #", "last snapshot date", "# of snapshot (success + fail = total (ratio))",
                         "last release build #", "last release date", "# of release (success + fail = total (ratio))", "UT coverage", "Property files", "Duplicate Keys"])
    for report in reports:
        csv_writer.writerow([
            report['job'],
            report["checkstyle_enabled"], report["pmd_enabled"], report["findbugs_enabled"],
